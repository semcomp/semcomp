...
// Atualizado para incluir as outras possíveis abas
type TabKey = "home" | "login" | "cronograma";
...
export default function Header() {
  const { width } = useWindowDimensions();
  const location = useLocation();
  const { isFeatureEnabled } = useFeatureFlags(); // <--- Hook adicionado

  // Filtro de abas baseado nas permissões do Banco de Dados
  const visibleTabs = allTabs.filter((tab) => isFeatureEnabled(tab.key));

  const activeTabObj = visibleTabs.find((t) => t.path === location.pathname);
  const active = activeTabObj ? activeTabObj.key : "home";

  // Tipagem dinâmica das referências dos botões para evitar erros de compilação
