...
type PaymentRepository interface {
	...
	SetProducts(paymentID uint, productIDs []uint) error
...
func (r *paymentRepository) SetProducts(paymentID uint, productIDs []uint) error {
	products := make([]product.Product, len(productIDs))
	for i, id := range productIDs {
		products[i] = product.Product{ID: id}
	}
	return r.db.Model(&Payment{ID_Payment: paymentID}).Association("Products").Replace(products)
}
