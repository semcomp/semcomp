 ...
 * @return {object}
 */
function UpdateEventModal({event, onRequestClose, onSuccess}) {
  ...
  async function handleSubmit(submitEvent) {
    ...
    setIsCreating(true);
    try {
      await API.updateEvent(event.id, newEvent);
      toast.success(`Evento '${event.name}' editado com sucesso!`);
      onSuccess(newEvent);
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <Modal style={{height: '100%', overflowY: 'auto'}} open onClose={onRequestClose}>
      <div className="w-full p-4 flex justify-center items-center">
        <div className="bg-white h-full rounded-lg p-4 flex flex-col items-center w-full max-w-lg">
          <h1 className="text-4xl">Atualizar Evento</h1>
          <EventForm
