import {
  FoodOption,
  TShirtSize,
} from "../components/profile/coffeePayment/coffee-step-2";
...
const Handlers = {
  ...
  events: {
    ...
    markAttendance: withNoErrorMessage((eventId: string) =>
      API.get(`/events/${eventId}`),
    ),
    markAttendanceByQrCode: withNoErrorMessage((token: string) =>
      API.post(`/events/mark-attendance`, { token }),
    ),
  },
  game: {
    ...
    getQuestion: (game, questionIndex) =>
      API.get(`/game/${game}/question/${questionIndex}`),
  },
  achievements: {
    getAchievements: withNoErrorMessage(() => API.get("/achievements")),
    readUserAchievementByQrCode: withNoErrorMessage((achievementId: string) =>
      API.post(`/achievements/${achievementId}/qrcode`),
    ),
  },
  config: {
    getConfig: withNoErrorMessage(() => API.get("/config")),
    checkCoffeeTotal: withNoErrorMessage(() => API.get("/config/coffee-total")),
    checkRemainingCoffee: withNoErrorMessage(() =>
      API.get("/config/coffee-remaining"),
    ),
  },
  coffee: {
    createPayment: (
    ...
    getPaymentInfo: withNoErrorMessage((id: string) =>
      API.get(`payments/user-id/${id}`),
    ),
    getPurchasedCoffees: withNoErrorMessage(() =>
      API.get("payments/purchased-coffees"),
    ),
    getAvailableTShirts: withNoErrorMessage(() =>
      API.get("payments/remaining-tshirts"),
    ),
  },
  sales: {
    getSales: withNoErrorMessage(() => API.get("/sales/get-sales")),
    getAvailableSales: withNoErrorMessage(() =>
      API.get("/sales/get-availables"),
    ),
  ...
  treasureHunt: {
    getImage: withNoErrorMessage((imageId: string) =>
      API.get(`/treasure-hunt-images/${imageId}`),
    ),
  },
};
