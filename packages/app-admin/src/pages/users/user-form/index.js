...
function UserForm({
  ...
  return (
    <form className="w-full p-4 flex flex-col" onSubmit={handleSubmit}>
      ...
      />
      {isCreating ? (
        <Input label="Password" name="password" type="text" fullWidth />
      ) : (
        <>
          <Input
            label="Course"
          ...
          />
          <FormControlLabel
            control={<Checkbox defaultChecked={initialData.paid} name="paid" />}
            label="---?"
          />
          <FormControlLabel
            control={
      ...
      <div className="w-full flex items-center justify-end pt-4">
        <ErrorButton onClick={onCancel} className="cancel" variant="contained">
          Cancelar
        </ErrorButton>
        <SuccessButton variant="contained" type="submit">
          {buttonLabel}{" "}
          {isLoading && (
            <Spinner color="white" strokeWidth={2} className="pl-2" />
          )}
        </SuccessButton>
      </div>
    </form>
