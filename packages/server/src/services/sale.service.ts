...
class SaleService {
  ...
  public async update(sale: Sale): Promise<Sale> {
    if (SaleType[sale.type] === SaleType.ITEM) {
      const allSales = (
        await this.findWithUsedQuantity({
          pagination: new PaginationRequest(0, 9999),
        })
      ).getEntities();

      const paymentWithItem = allSales.find(
        (s) => s.id === sale.id,
      )?.usedQuantity;
      if (paymentWithItem > sale.quantity) {
        throw new HttpError(400, [
          `A quantidade não pode ser menor do que a já comprada (${paymentWithItem})!`,
        ]);
      }

      // Atualiza a quantidade das vendas que usam o item
      for (const s of allSales.filter(
        (s) => SaleType[s.type] === SaleType.SALE,
      )) {
        const allQuantities = s.items
          .filter((item) => item !== sale.id)
          .map((item) => allSales.find((p) => p.id === item).quantity);
        const least = Math.min(...allQuantities);
        const bigger = Math.max(...allQuantities);

        if (
          s.items.includes(sale.id) &&
          (sale.quantity < least ||
            sale.quantity <= bigger ||
            sale.items.length === 1)
        ) {
          s.updatedAt = Date.now();
          const entity = await SaleModel.findOneAndUpdate(
            { id: s.id },
    ...
    if (SaleType[sale.type] === SaleType.ITEM) {
      ...
      for (const s of sales.getEntities()) {
        if (s.items.includes(sale.id)) {
          throw new HttpError(400, [
            `O item não pode ser deletado pois está sendo usado em uma venda!`,
          ]);
        }
      }
    }
  ...
  public async findWithUsedQuantity({
    ...
    const sales = allSales.filter(
      (sale) => SaleType[sale.type] === SaleType.SALE,
    );
    const items = allSales.filter(
      (sale) => SaleType[sale.type] === SaleType.ITEM,
    );
    ...
    for (const sale of sales) {
      let countSale = await new PaymentServiceImpl(
        null,
        null,
        null,
        null,
      ).count({ salesOption: [sale.id], status: PaymentStatus.APPROVED });
      countSale += await new PaymentServiceImpl(null, null, null, null).count({
        salesOption: [sale.id],
    ...
    return new PaginationResponse<Sale & { usedQuantity: number }>(
      entities,
  ...
  public async getItems(): Promise<Sale[]> {
    ...
    if (sales) {
      const items = sales
        .getEntities()
        .filter((sale) => SaleType[sale.type] === SaleType.ITEM);
      return items;
    }
  ...
  public async getAvailables(): Promise<(Sale & { usedQuantity: number })[]> {
    const sales = await this.findWithUsedQuantity({
      pagination: new PaginationRequest(0, 9999),
    });
    const withoutItems = sales
      .getEntities()
      .filter((sale) => SaleType[sale.type] === SaleType.SALE);
