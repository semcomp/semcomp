...
const { formatEvent } = require("../lib/format-event");
...
const eventService = {
  ...
  getCurrent: async (user) => {
    ...
    for (const event of events) {
      ...
      const startedDateObj = new Date(event.startDate);
  ...
  subscribe: async (eventId, user, info) => {
    ...
    await subscriptionService.create(user.id, event.id, info);
