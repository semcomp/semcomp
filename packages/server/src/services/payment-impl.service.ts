...
export default class PaymentServiceImpl implements PaymentService {
  ...
  public async findByUserId(id: string): Promise<Payment> {
    ...
    if (entity && entity.length > 0) {
      return entity.map((payment) =>
        this.mapEntity(payment as Model<Payment> & Payment),
      );
    }
  ...
  public async update(payment: Payment): Promise<Payment> {
    payment.updatedAt = Date.now();
    const entity = await PaymentModel.findOneAndUpdate(
      { id: payment.id },
      payment,
    );
  ...
  public async getAvailableTShirts(): Promise<Object> {
    ...
    allAvailableTShirts.map((tShirt) => {
      availableTShirts[tShirt.size] += tShirt.quantity;
    });

    return availableTShirts;
  }

  public findItemsInSales(
    saleOptionItemsIds: string[],
  ...
  public async createPayment(
    ...
    if (config === undefined) {
      throw new HttpError(401, [
        "Erro ao acessar as configurações de ambiente!",
      ]);
    ...
    const userPayments = await this.find({
      userId,
      status: [PaymentStatus.APPROVED, PaymentStatus.PENDING],
    });
    const salesDoc = await SaleService.find({
    ...
    if (needTshrit && tShirtSize !== TShirtSize.NONE) {
      ...
      let paymentsWithThisTShirtSize = await this.count({
        tShirtSize,
        status: PaymentStatus.APPROVED,
      });
      paymentsWithThisTShirtSize += await this.count({
        tShirtSize,
    ...
    const saleOptionItemsIds = salesOptionObjects
      .map((sale) => sale.items)
      .flat();
    const duplicates = saleOptionItemsIds.filter(
      (item, index) => saleOptionItemsIds.indexOf(item) !== index,
    );
    const conflictOldPayments = this.findItemsInSales(
      saleOptionItemsIds,
      salesPaymentObject.map((sale: Sale) => sale.items).flat(),
    );
    if (duplicates.length > 0) {
      throw new HttpError(400, ["Você tem itens repetidos!"]);
    } else if (conflictOldPayments) {
      throw new HttpError(400, [
        "Pelo menos um dos itens já foi adquirido em compras anteriores!",
      ]);
    }
  ...
  public async receive(id: number): Promise<void> {
    ...
    const paymentResponse = await this.paymentIntegrationService.find(
      payment.paymentIntegrationId,
    );
    ...
    if (payment.status === PaymentStatus.CANCELED) {
      ...
      if (paymentResponse.status === "approved") {
        const response = await this.paymentIntegrationService.refund(
          payment.paymentIntegrationId,
          payment.amount,
        );
        if (response) await this.delete(payment);
      } else {
        const response = await this.paymentIntegrationService.cancel(
          payment.paymentIntegrationId,
        );
        if (response) await this.delete(payment);
      }
  ...
  public async getUserPayment(userId: string): Promise<Payment[]> {
    ...
    const approvedPayment = userPayments.filter(
      (userPayment: Payment) => userPayment.status === PaymentStatus.APPROVED,
    );
    ...
    const pendingPayment = userPayments.filter(
      (userPayment: Payment) => userPayment.status === PaymentStatus.PENDING,
    );
  ...
  async generateQrCodes(): Promise<void> {
    ...
    for (const user of users.getEntities()) {
      ...
      // }
      await QRCode.toFile(
        `./qr-code/${user.email} - ${user.name}.png`,
        user.id,
        {
          color: {
            dark: "#000000",
