...
import DonationForm, { DonationFormData } from "./DonationForm";

function CreateDonationModal({ onRequestClose }) {
  const {
    semcompApi
  }: {
  ...
  const [data, setData] = useState(null as DonationFormData);
  const [isLoading, setIsLoading] = useState(false);

  async function handleSubmit() {
    try {
      setIsLoading(true);
      await semcompApi.createDonation(data);
  ...
  return (
    <Modal onRequestClose={onRequestClose}>
      ...
      <div className="max-h-96 overflow-y-scroll p-6 w-full">
        <DonationForm onDataChange={setData}></DonationForm>
      </div>
